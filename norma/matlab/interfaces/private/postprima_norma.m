function [x, fx, exitflag, output] = postprima_norma(probinfo, output)
%POSTPRIMA postprocesses the output by prima_norma or its solvers and creates the
%   output variables.
%
%   ***********************************************************************
...
% N.B.: probinfo = [] if the invoker is a solver called by prima_norma. In that case, postprima_norma will return
% after verifying `output`, and then prima_norma will call postprima_norma again (after assigning `probinfo`) to
...
invoker_list = ['prima_norma', all_solvers()];
callstack = dbstack;
...
if (length(callstack) >= 3) && strcmp(callstack(3).name, 'prima_norma')
    % In this case, preprima_norma sets probinfo to empty.
    if ~isempty(probinfo)
        % Public/unexpected error
        error(sprintf('%s:InvalidProbinfo', invoker),...
            '%s: UNEXPECTED ERROR: probinfo should be empty because %s is a solver called by prima_norma.', invoker, invoker);
    end
else
...
if strcmp(invoker, 'prima_norma')
    % In this case, the invoker is prima_norma rather than a solver called by prima_norma.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If the invoker is a solver called by prima_norma, then let prima_norma do the postprocessing.
...
if nf <= 0
    % If preprima_norma works properly, then nf <= 0 should never happen.
    % Public/unexpected error
    error(sprintf('%s:InvalidNF', invoker), ...
...
if ~options.classical && ~probinfo.infeasible && ~probinfo.nofreex
    ...
    if strcmp(solver, 'cobyla_norma') && (any(chist > constrmax) || any(isnan(chist)))
        % Public/unexpected error
        error(sprintf('%s:InvalidChist', invoker), ...
...
switch exitflag % If preprima_norma works properly, then 5, 6, 10, 11, 12 should never happen
case 0
...
% Recover the default warning behavior of displaying stack trace, which was disabled by prima_norma or its solvers
...
if options.debug && ~options.classical
    ...
    if strcmp(solver, 'bobyqa_norma') && (max([chist, constrviolation]) > 0) && ~probinfo.infeasible
        % Public/unexpected error
        error(sprintf('%s:InvalidChist', invoker), ...
             '%s: UNEXPECTED ERROR: %s is a feasible solver yet it returns a positive constrviolation.', invoker, solver);
    end
    if strcmp(options.precision, 'double') && (strcmp(solver, 'lincoa_norma') || strcmp(solver, 'cobyla_norma'))
        ...
        if ~(isnan(cstrv) && isnan(constrviolation)) && ~(cstrv == inf && constrviolation == inf) && ...
                ~(abs(constrviolation-cstrv) <= lincoa_norma_prec*max(1, cstrv) && strcmp(solver, 'lincoa_norma')) && ...
                ~(abs(constrviolation-cstrv) <= cobyla_norma_prec*max(1, cstrv) && strcmp(solver, 'cobyla_norma'))
            % Public/unexpected error
            error(sprintf('%s:InvalidConstrViolation', invoker), ...
    ...
    if options.chkfunval % Check the values of fun(x) and con(x)
        ...
        %if ~(isnan(fx) && isnan(funx)) && ~((fx == funx) || (abs(funx-fx) <= cobyla_norma_prec*max(1, abs(fx)) && strcmp(solver, 'cobyla_norma')))
        % Zaikun 20220930: It seems that BOBYQA can also return fx ~= fun(x) if RESCUE is invoked.
        if ~(isnan(fx) && isnan(funx)) && ~((fx == funx) || (abs(funx-fx) <= bobyqa_norma_prec*max(1, abs(fx)) ...
                && strcmp(solver, 'bobyqa_norma')) || (abs(funx-fx) <= cobyla_norma_prec*max(1, abs(fx)) && strcmp(solver, 'cobyla_norma')))
            % Public/unexpected error
            error(sprintf('%s:InvalidFx', invoker), ...
        ...
        if any(size([nlcineq; nlceq]) ~= size([nlcineqx; nlceqx])) || any(isnan([nlcineq; nlceq]) ~= isnan([nlcineqx; nlceqx])) || (~any(isnan([nlcineq; nlceq; nlcineqx; nlceqx])) && any(abs([0; nlcineq; nlceq] - [0; nlcineqx; nlceqx]) > cobyla_norma_prec*max(1,abs([0; nlcineqx; nlceqx]))))
        ...
        if ~isempty(xhist) && (isfield(output, 'nlcihist') || isfield(output, 'nlcehist') || isfield(output, 'chist'))
            ...
            if isfield(output, 'nlcihist') || isfield(output, 'nlcehist')
                if any(size([nlcihist; nlcehist]) ~= size([nlcihistx; nlcehistx])) || ...
                        ...
                        > cobyla_norma_prec*max(1,abs([zeros(1, nhist); nlcihistx; nlcehistx])), 'all'))
                    % In the last few max of the above line, we put a 0 to avoid an empty result
                    % Public/unexpected error
                    error(sprintf('%s:InvalidConx', invoker), ...
            ...
            if strcmp(solver, 'lincoa_norma') || strcmp(solver, 'cobyla_norma')
                ...
                if any(~(isnan(chist) & isnan(chistx)) & ~((chist == chistx) | ...
                        (abs(chistx-chist) <= lincoa_norma_prec*max(1, chist) & strcmp(solver, 'lincoa_norma')) | ...
                        (abs(chistx-chist) <= cobyla_norma_prec*max(1, chist) & strcmp(solver, 'cobyla_norma'))))
                    % Public/unexpected error
                    error(sprintf('%s:InvalidFx', invoker), ...

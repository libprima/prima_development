function [x, fx, exitflag, output] = newuoa_norma(varargin)
...
%
%   [x, fx, exitflag, output] = newuoa_norma(INPUTS)
%
...
%       solver: backend solver that does the computation, i.e., 'newuoa_norma'
...
%   x = newuoa_norma(problem);
%
%   solves
%       min cos(x)
%   starting from x0 = -1 with at most 50 function evaluations.
%
%   See also prima_norma, uobyqa_norma, bobyqa_norma, lincoa_norma, cobyla_norma.
%
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:2), cell(1, 7), varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_norma
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
...
try % preprima_norma is a private function that may generate public errors; error-handling needed
    [fun, x0, ~, ~, ~, ~, ~, ~, ~, options, probinfo] = preprima_norma(args{:});
catch exception
...
if ~strcmp(invoker, 'prima_norma') && probinfo.feasibility_problem
    ...
    try
        if options.fortran
            ...
            [x, fx, exitflag, nf, xhist, fhist] = ...
                newuoa_norma_mat.newuoa_norma_mat(fun, x0, rhobeg, rhoend, eta1, eta2, gamma1, gamma2, ftarget, maxfun, npt, ...

...
!
! Last Modified: Wednesday, May 31, 2023 AM12:16:32
!--------------------------------------------------------------------------------------------------!
...
normg = norm(glag)
!if (normg > 0) then
if (normg > EPS .and. normg <= huge(normg)) then
...
normg = norm(pglag)
if (nact > 0 .and. nact < n .and. normg > EPS .and. normg <= huge(normg)) then  ! EPS prevents floating point exception.

...
if isempty(requirements.list)
    ...
    case {'uobyqa', 'uobyqan'}
        blacklist = [blacklist, {'MUONSINELS'}]; % 1-D problem, which is not supported by Powell's UOBYQA
        blacklist = [blacklist, {'YFITU'}]; % Takes too long
        blacklist = [blacklist, {'VARDIM', 'HIELOW', 'HATFLDFL', 'BENNETT5LS'}];  % too large for UOBYQA
    case {'newuoa', 'newuoan'}
          blacklist = [blacklist, {'ARGTRIGLS', 'BROWNAL', 'VARDIM', 'LUKSAN21LS'}]; % More than 30 minutes to solve.
    ...
    case {'lincoa', 'lincoan'}
        ...
        blacklist = [blacklist, {'CHEBYQAD', 'HIMMELBJ', 'HS54'}]; % The classical lincoa encounters segfault
        blacklist = [blacklist, {'ARGTRIGLS', 'BROWNAL', 'PENALTY3', 'VARDIM'}]; % More than 10 minutes to solve.
        blacklist = [blacklist, {' QPNBOEI2', 'QPCBOEI2', 'EXTRASIM', 'SIM2BQP', 'BQP1VAR', 'LUKSAN22LS','LUKSAN21LS', 'MINSURF', 'QPCBLEND', 'MUONSINELS', 'LSQFIT', 'CVXQP1'}]; % Too long to solve
    case {'cobyla', 'cobylan'}
        blacklist = [blacklist, {'HS80', 'HS77'}];  % QRADD_RDIAG: Assertion failed: C^T*Q(:, N) == Rdiag(N).
        blacklist = [blacklist, {'POLAK6', 'SPIRAL', 'POLAK2'}]; % B = A^{-1} fails
        blacklist = [blacklist, {'PALMER4ANE', 'PALMER5BNE'}];
        blacklist = [blacklist, {'LSNNODOC', 'DEGENLPB', 'RES', 'AVION2', 'SIPOW3', 'HS55','PRODPL1','BQPGASIM','LSNNODOC', 'VESUVIA', 'MESH', 'LOADBAL','MIFFLIN1', 'ACOPR14', 'PALMER1NE','PALMER8ENE', 'DUALC5'}];  % Takes long to solve
        ...
        %blacklist = [blacklist, {'DUAL2', 'FEEDLOC', 'GROUPING', 'HYDCAR20', 'PALMER7ANE', 'LINSPANH', 'LUKSAN11', ...
        ...
        blacklist = [blacklist, {'HYDCAR20', 'PALMER7ANE', 'LUKSAN13', 'CHEBYQADNE', 'HAIFAM', 'LUKSAN12', 'HIMMELBI', 'DUAL1', ...
        ...
        'ERRINRSMNE', 'FBRAIN3', 'FEEDLOC', 'GROUPING', 'HAIFAM', 'HIMMELBI', 'HYDC20LS', 'HYDCAR20', 'PALMER7ANE', 'KISSING2', ...
        ...
        %'ERRINRSMNE', 'FBRAIN3', 'HAIFAM', 'HIMMELBI', 'HYDC20LS', 'HYDCAR20', 'PALMER7ANE', ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
test_options.classical = ~(isfield(options, 'no_classical') && options.no_classical) && (rand < 0.2)&& test_ready_solvers;

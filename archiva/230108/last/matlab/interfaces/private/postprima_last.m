function [x, fx, exitflag, output] = postprima_last(probinfo, output)
%POSTPRIMA postprocesses the output by prima_last or its solvers and creates the
%   output variables.
%
%   ***********************************************************************
...
% N.B.: probinfo = [] if the invoker is a solver called by prima_last. In that case, postprima_last will return
% after verifying `output`, and then prima_last will call postprima_last again (after assigning `probinfo`) to
...
invoker_list = ['prima_last', all_solvers()];
callstack = dbstack;
...
% Verify probinfo
if (length(callstack) >= 3) && strcmp(callstack(3).name, 'prima_last')
    % In this case, preprima_last sets probinfo to empty.
    if ~isempty(probinfo)
        % Public/unexpected error
        error(sprintf('%s:InvalidProbinfo', invoker),...
            '%s: UNEXPECTED ERROR: probinfo should be empty because %s is a solver called by prima_last.', invoker, invoker);
    end
else
...
if strcmp(invoker, 'prima_last')
    % In this case, the invoker is prima_last rather than a solver called by prima_last.
...
if strcmp(solver, 'lincoa_last')
    % For lincoa_last, output should contain constr_modified
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If the invoker is a solver called by prima_last, then let prima_last do the postprocessing.
...
if nf <= 0
    % If preprima_last works properly, then nf <= 0 should never happen.
    % Public/unexpected error
    error(sprintf('%s:InvalidNF', invoker), ...
...
% Recover the default warning behavior of displaying stack trace, which was disabled by prima_last or its solvers
...
if options.debug && ~options.classical && strcmp(options.precision, 'double')
    ...
    minf = min([fhistf, fx]);
    if (fx ~= minf) && ~(isnan(fx) && isnan(minf)) && ~(strcmp(solver, 'lincoa_last') && constr_modified)
        % Public/unexpected error
        error(sprintf('%s:InvalidFhist', invoker), ...
    ...
    cobyla_last_prec = 1e-6;
    lincoa_last_prec = 1e-9;
    bobyqa_last_prec = 1e-9;
    ...
    % relative error is within cobyla_last_prec.
    ...
    if strcmp(solver, 'bobyqa_last') && (max([chist, constrviolation]) > 0) && ~probinfo.infeasible
        % Public/unexpected error
        error(sprintf('%s:InvalidChist', invoker), ...
             '%s: UNEXPECTED ERROR: %s is a feasible solver yet it returns a positive constrviolation.', invoker, solver);
    end
    if strcmp(options.precision, 'double') && ((strcmp(solver, 'lincoa_last') && ~constr_modified) || strcmp(solver, 'cobyla_last'))
    ...
    if options.chkfunval % Check the values of fun(x) and con(x)
        ...
        %if ~(isnan(fx) && isnan(funx)) && ~((fx == funx) || (abs(funx-fx) <= cobyla_last_prec*max(1, abs(fx)) && strcmp(solver, 'cobyla_last')))
        % Zaikun 20220930: It seems that BOBYQA can also return fx ~= fun(x) if RESCUE is invoked.
        if ~(isnan(fx) && isnan(funx)) && ~((fx == funx) || (abs(funx-fx) <= bobyqa_last_prec*max(1, abs(fx)) && strcmp(solver, 'bobyqa_last')) || (abs(funx-fx) <= cobyla_last_prec*max(1, abs(fx)) && strcmp(solver, 'cobyla_last')))
            % Public/unexpected error
            error(sprintf('%s:InvalidFx', invoker), ...
        ...
        if ~isempty(xhist) && (isfield(output, 'nlcihist') || isfield(output, 'nlcehist') || isfield(output, 'chist'))
            ...
            if isfield(output, 'nlcihist') || isfield(output, 'nlcehist')
                if any(size([nlcihist; nlcehist]) ~= size([nlcihistx; nlcehistx])) || ...
                        ...
                        > cobyla_last_prec*max(1,abs([zeros(1, nhist); nlcihistx; nlcehistx])), 'all'))
                    % In the last few max of the above line, we put a 0 to avoid an empty result
                    % Public/unexpected error
                    error(sprintf('%s:InvalidConx', invoker), ...
            ...
            if (strcmp(solver, 'lincoa_last') && ~constr_modified) || strcmp(solver, 'cobyla_last')

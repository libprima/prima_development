function [x, fx, exitflag, output] = prima_norma(varargin)
...
%
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
%
...
%
%   [x, fx, exitflag, output] = prima_norma(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = prima_norma(fun, x0) solves
%       minimize fun(x)
%   x = prima_norma(fun, x0, Aineq, bineq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq, lb) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x <= ub
%   x = prima_norma(fun, x0, nonlcon) solves
%       minimize fun(x) s.t. cineq(x) <= 0, ceq(x) = 0
%   x = prima_norma(fun, x0, Aineq, bineq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, cineq(x) <= 0, ceq(x) = 0
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, cineq(x) <= 0, ceq(x) = 0
%   x = prima_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x, cineq(x) <= 0, ceq(x) = 0
%
%   information = prima_norma(request) returns information about the package
...
%       'uobyqa_norma', 'newuoa_norma' (for unconstrained problems),
%       'bobyqa_norma' (for bound-constrained or unconstrained problems),
%       'lincoa_norma' (for linearly-constrained or bound-constrained or
%       unconstrained problems),
%       'cobyla_norma' (for general constrained or unconstrained problems)
...
%   options = struct();
%   options.maxfun = 50;
%   x = prima_norma(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 * x <= 3
...
%
%   See https://www.prima_norma.net for more information.
%
%   ***********************************************************************
...
% They should be either prima_norma or a solver.
...
% auxiliary functions.
%
% 1. Errors that may be generated by prima_norma
%
...
% (should be either prima_norma or a solver). Thanks to the error-handling
...
% function that he/she is calling. Therefore, if the user calls prima_norma,
% then PUB_FUN should be prima_norma; if the user calls a solver directly, then
...
% 2. Change the interface of preprima_norma to
%    probinfo = preprima_norma(argin{:}, interface_type),
...
% 3. To add a new solver, we only need to call preprima_norma, call the solver
%    using the information in probinfo, record the results in output, and
%    then call postprima_norma.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% This version of prima_norma.m produces no warning. However, initializing output.warnings
% is still necessary, as output.warnings is required by postprima_norma.
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    if isempty(varargin{end}) || isa(varargin{end}, 'struct')
        ...
        if (nvararg >= 4) && (ischarstr(varargin{end-1}) || isa(varargin{end-1}, 'function_handle'))
            args = [varargin(1:end-2), cell(1, maxarg-nvararg), varargin(end-1:end)]; % 'augment' the inputs to 10 by adding []; args{:} (should have 10 entries) will be the inputs for preprima_norma
...
try % preprima_norma and package_info are private functions that may generate public errors; error-handling needed
    if (nvararg == 1) && ischarstr(varargin{1})
        ...
        [fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon, options, probinfo] = preprima_norma(args{:});
    end
catch exception
...
if probinfo.infeasible % The problem turned out infeasible during preprima_norma
    output.x = x0;
    ...
    if strcmp(options.solver, 'lincoa_norma') % LINCOA requires constr_modified to exist in output
        output.constr_modified = false;
    end
elseif probinfo.nofreex % x was fixed by the bound constraints during preprima_norma
...
elseif probinfo.feasibility_problem && ~strcmp(probinfo.refined_type, 'nonlinearly-constrained')
    output.x = x0;  % preprima_norma has tried to set x0 to a feasible point (but may have failed)
    ...
    % and fhist in postprima_norma. To avoid such a modification, we set fx, funcCount,
    % and fhist as below and then revise them in postprima_norma.
    output.fx = fun(output.x);  % preprima_norma has defined a fake objective function
    output.funcCount = 1;
    if options.output_xhist
        output.xhist = output.x;
    end
    ...
    output.nlceq = [];
    if output.constrviolation < eps  % Did preprima_norma find a feasible point?
        output.exitflag = 14;
    else
        output.exitflag = 15;
    end
    ...
    % The problem turns out 'normal' during preprima_norma. Solve it by
    % options.solver, which has been defined in preprima_norma.
    try
        switch lower(options.solver)
        case 'uobyqa_norma'
            [x, fx, exitflag, output] = uobyqa_norma(fun, x0, options);
        case 'newuoa_norma'
            [x, fx, exitflag, output] = newuoa_norma(fun, x0, options);
        case 'bobyqa_norma'
            [x, fx, exitflag, output] = bobyqa_norma(fun, x0, lb, ub, options);
        case 'lincoa_norma'
            [x, fx, exitflag, output] = lincoa_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, options);
        case 'cobyla_norma'
            [x, fx, exitflag, output] = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon, options);
        end
    catch exception
        if ~isempty(regexp(exception.identifier, sprintf('^%s:', funname), 'once')) % Public error; displayed friendly
            error(exception.identifier, '%s\n(error generated in %s, line %d)', exception.message, exception.stack(1).file, exception.stack(1).line);

...
if strcmp(solver, 'lincoa_norma')
    % For lincoa_norma, output should contain constr_modified
...
if options.debug && ~options.classical && strcmp(options.precision, 'double')
    ...
    if (fx ~= minf) && ~(isnan(fx) && isnan(minf)) && ~(strcmp(solver, 'lincoa_norma') && constr_modified) && ~strcmp(options.precision, 'quadruple')
        % Public/unexpected error
        error(sprintf('%s:InvalidFhist', invoker), ...
    ...
    if options.chkfunval % Check the values of fun(x) and con(x)
        ...
        if ~(isnan(fx) && isnan(funx)) && ~((fx == funx) || (abs(funx-fx) <= bobyqa_norma_prec*max(1, abs(fx)) && strcmp(solver, 'bobyqa_norma')) || (abs(funx-fx) <= cobyla_norma_prec*max(1, abs(fx)) && strcmp(solver, 'cobyla_norma')))
            % Public/unexpected error
            error(sprintf('%s:InvalidFx', invoker), ...
        ...
        if ~isempty(xhist) && (isfield(output, 'nlcihist') || isfield(output, 'nlcehist') || isfield(output, 'chist'))
            ...
            if (strcmp(solver, 'lincoa_norma') && ~constr_modified) || strcmp(solver, 'cobyla_norma')
                ...
                if any(~(isnan(chist) & isnan(chistx)) & ~((chist == chistx) | ...
                        ...
                        (abs(chistx-chist) <= cobyla_norma_prec*max(1, abs(chist)) & strcmp(solver, 'cobyla_norma'))))
                    % Public/unexpected error
                    error(sprintf('%s:InvalidFx', invoker), ...

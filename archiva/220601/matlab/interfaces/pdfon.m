function [x, fx, exitflag, output] = pdfon(varargin)
...
%       'uobyqan', 'newuoan' (for unconstrained problems),
%       'bobyqan' (for bound-constrained or unconstrained problems),
...
elseif probinfo.feasibility_problem && ~strcmp(probinfo.refined_type, 'nonlinearly-constrained')
    ...
    try
        ...
        case 'uobyqan'
            [x, fx, exitflag, output] = uobyqan(fun, x0, options);
        case 'newuoan'
            [x, fx, exitflag, output] = newuoan(fun, x0, options);
        case 'bobyqan'
            [x, fx, exitflag, output] = bobyqan(fun, x0, lb, ub, options);
        case 'lincoan'
            [x, fx, exitflag, output] = lincoan(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, options);
        case 'cobylan'
            [x, fx, exitflag, output] = cobylan(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon, options);
        end
    catch exception
        if ~isempty(regexp(exception.identifier, sprintf('^%s:', funname), 'once')) % Public error; displayed friendly
            error(exception.identifier, '%s\n(error generated in %s, line %d)', exception.message, exception.stack(1).file, exception.stack(1).line);

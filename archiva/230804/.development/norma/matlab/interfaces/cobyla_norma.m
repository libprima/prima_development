function [x, fx, exitflag, output] = cobyla_norma(varargin)
...
%
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
%
...
%
%   [x, fx, exitflag, output] = cobyla_norma(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = cobyla_norma(fun, x0) solves
%       minimize fun(x)
%   x = cobyla_norma(fun, x0, Aineq, bineq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, lb) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x <= ub
%   x = cobyla_norma(fun, x0, nonlcon) solves
%       minimize fun(x) s.t. cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_norma(fun, x0, Aineq, bineq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, nonlcon) solves
...
%       solver: backend solver that does the computation, i.e., 'cobyla_norma'
...
%   options = struct();
%   options.maxfun = 50;
%   x = cobyla_norma(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 * x <= 3
...
internal_invokers = {'prima_norma'}; % Invokers from this package; may have others in the future
...
% then postprima_norma will do nothing; the real postprocessing will be done when
% prima_norma calls postprima_norma using the OUTPUT returned by solver together with the
% PROBINFO in prima_norma; that said, in such a scenario, the PROBINFO of this solver
% will NOT be passed to the real postprocessing. Indeed, the PROBINFO of
% this solver is set to empty in preprima_norma.
...
% Even if invoker = 'prima_norma', we still need to call preprima_norma, which will assign
% values to fun, x0, ..., options.
try % preprima_norma is a private function that may generate public errors; error-handling needed
...
elseif ~strcmp(invoker, 'prima_norma') && probinfo.nofreex % x was fixed by the bound constraints during preprima_norma
...
elseif ~strcmp(invoker, 'prima_norma') && probinfo.feasibility_problem && ~strcmp(probinfo.refined_type, 'nonlinearly-constrained')
...
else % The problem turns out 'normal' during preprima_norma
    % Include all the constraints into one single 'nonlinear constraint'
    %funcon = @(x) cobyla_norma_funcon(x, fun, Aineq, bineq, Aeq, beq, lb, ub, nonlcon);
    ...
    try
        funcon_norma = @(x) cobyla_norma_funcon(x, fun, Aineq, bineq, Aeq, beq, lb, ub, nonlcon);
        [x, fx, constrviolation, constr, exitflag, nf, xhist, fhist, chist, conhist] = ...
            ...
            double(output_xhist), double(output_nlchist), maxfilt, funcon_norma);
        %[x, fx, constrviolation, constr, exitflag, nf, xhist, fhist, chist, conhist] = ...
...
%%%%%%%%%%%%%%%%%%%%%%%%% Auxiliary functions %%%%%%%%%%%%%%%%%%%%%
function [f, constr, m_nlcineq, m_nlceq] = cobyla_norma_funcon(x, fun, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
f = fun(x);
[constr, m_nlcineq, m_nlceq] = cobyla_norma_con(x, Aineq, bineq, Aeq, beq, lb, ub, nonlcon);
return

function [constr, m_nlcineq, m_nlceq] = cobyla_norma_con(x, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)

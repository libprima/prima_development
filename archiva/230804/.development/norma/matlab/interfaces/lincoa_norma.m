function [x, fx, exitflag, output] = lincoa_norma(varargin)
...
%
%   x = lincoa_norma(fun, x0, Aineq, bineq, Aeq, beq, lb, ub)
%
...
%
%   [x, fx, exitflag, output] = lincoa_norma(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = lincoa_norma(fun, x0) solves
%       minimize fun(x)
%   x = lincoa_norma(fun, x0, Aineq, bineq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq
%   x = lincoa_norma(fun, x0, Aineq, bineq, Aeq, beq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq
%   x = lincoa_norma(fun, x0, Aineq, bineq, Aeq, beq, lb) solves
...
%       solver: backend solver that does the computation, i.e., 'lincoa_norma'
...
%   options = struct();
%   options.maxfun = 50;
%   x = lincoa_norma(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 * x <= 3
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:8), {[]}, varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_norma
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
...
try % preprima_norma is a private function that may generate public errors; error-handling needed
    [fun, x0, Aineq, bineq, Aeq, beq, lb, ub, ~, options, probinfo] = preprima_norma(args{:});
catch exception
...
elseif ~strcmp(invoker, 'prima_norma') && probinfo.nofreex % x was fixed by the bound constraints during preprima_norma
    ...
    output.chist = output.constrviolation;
    output.constr_modified = false;
elseif ~strcmp(invoker, 'prima_norma') &&  probinfo.feasibility_problem
    ...
    A_aug = [-Alb; Aub; -Aeq; Aeq; Aineq]';
    b_aug = [-lb(:); ub(:); -beq(:); beq(:); bineq];
    ...
    % preprima_norma has tried to make find a feasible x0. Raise a warning is

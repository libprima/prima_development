function [x, fx, exitflag, output] = bobyqa_norma(varargin)
...
%
%   [x, fx, exitflag, output] = bobyqa_norma(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = bobyqa_norma(fun, x0) solves
%       minimize fun(x)
%   x = bobyqa_norma(fun, x0, lb) solves
...
%       solver: backend solver that does the computation, i.e., 'bobyqa_norma'
...
%   options = struct();
%   options.maxfun = 50;
%   x = bobyqa_norma(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 <= x <= 3
...
%   x = bobyqa_norma(problem);
%
%   solves
%       min cos(x) s.t. 2 <= x <= 3
%   starting from x0 = -1 with at most 50 function evaluations.
%
%   See also prima_norma, uobyqa_norma, newuoa_norma, lincoa_norma, cobyla_norma.
%
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:2), cell(1, 4), varargin(3:4), {[]}, varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_norma
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
...
try % preprima_norma is a private function that may generate public errors; error-handling needed
    [fun, x0, ~, ~, ~, ~, lb, ub, ~, options, probinfo] = preprima_norma(args{:});
catch exception
...
elseif ~strcmp(invoker, 'prima_norma') && probinfo.feasibility_problem
    ...
    output.x = x0;  % preprima_norma has set x0 to a feasible point
